...
This RFC proposes adding first-class support for Swift Package Manager (SPM) for React Native on Apple platforms, with the goal of replacing CocoaPods over a staged transition period.

With CocoaPods deprecation targeted for December 2026, React Native needs a supported SPM-based integration story for React Native core, Hermes, Codegen, autolinking, and third-party native libraries.
...
- Some Apple ecosystem dependencies are now distributed primarily through Swift Package Manager.
...
~~~bash
node node_modules/react-native/scripts/setup-ios-spm.js
~~~
...
The prototype uses a single setup script, `setup-ios-spm.js`, to prepare an app for this layout. That script runs Codegen, generates SPM autolinking, downloads the required artifacts, generates the package manifests, and creates an Xcode project configured for the SPM flow.

From the app’s point of view, the integration stays relatively simple: the app’s `Package.swift` depends on three local generated packages for React Native core, generated code, and autolinked native modules. The generated Xcode project also includes an auto-sync build phase that refreshes autolinking when dependencies change.
...
- a generated Clang VFS overlay for the remaining non-standard include cases
...
A possible shape for library metadata is:
...
- The prototype currently relies on a generated Xcode project with build phases for setup, syncing, and bundling, rather than a purely package-only SPM workflow.
...
This keeps the current model intact, but it preserves the Ruby/CocoaPods dependency and leaves React Native without a credible migration path ahead of the targeted December 2026 deprecation.
...
- Prefer prebuilt artifacts for heavy native dependency graphs, while keeping source-based integration available where practical.
